...
def cnn_model(input):
    # 第一层卷积
    conv1 = Convolution2D(filters=32, kernel_size=(3, 3), padding='same')(input)
    bath_normal1 = BatchNormalization()(conv1)
    dropout1 = Dropout(0.2)(bath_normal1)
    activate1 = Activation('relu')(dropout1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(activate1)

    # 第二层卷积
    conv2 = Convolution2D(filters=32, kernel_size=(3, 3), padding='same')(pool1)
    bath_normal2 = BatchNormalization()(conv2)
    dropout2 = Dropout(0.2)(bath_normal2)
    activate2 = Activation('relu')(dropout2)
    pool2= MaxPooling2D(pool_size=(2, 2))(activate2)

    # 第s三层卷积
    conv3 = Convolution2D(filters=32, kernel_size=(3, 3), padding='same')(pool2)
    bath_normal3 = BatchNormalization()(conv3)
    dropout3 = Dropout(0.2)(bath_normal3)
    activate3 = Activation('relu')(dropout3)
    pool3 = MaxPooling2D(pool_size=(2, 2))(activate3)
